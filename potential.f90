...
!   !-------------------------
  ...
  ! !LJ ------------------
   ES = ES + K_ARRAY(BONDTYPE)*((R_ARRAY(BONDTYPE)**2)/72)*((R_ARRAY(BONDTYPE)/R)**12-2*(R_ARRAY(BONDTYPE)/R)**6)
   ENDDO
...
!   !2) Stretching gradient
  ...
  !LJ -------------------
TEMP = 12 * K_ARRAY(BONDTYPE) * ((R_ARRAY(BONDTYPE)**2)/72) * ( (R_ARRAY(BONDTYPE)**6 / R**7) - (R_ARRAY(BONDTYPE)**12 / R**13) ) * ( (COORDS(NODE1,1)-COORDS(NODE2,1)) / R )
    !--------------------
  ...
  !LJ -----------------
TEMP = 12.0 * K_ARRAY(BONDTYPE) * ((R_ARRAY(BONDTYPE)**2)/72) * ( (R_ARRAY(BONDTYPE)**6 / R**7) - (R_ARRAY(BONDTYPE)**12 / R**13) ) * ( (COORDS(NODE1,2)-COORDS(NODE2,2)) / R )
  !--------------------
...
DO i=1,N_ROD
  DO j=1, N_SEG+1
    CUR = (i-1)*(N_SEG+1) + j
    GRAD_L(CUR)=GRAD_L(CUR)+(12*KL*(LS_0**2)/72)*(-(LS_0**12)/(LENGTHS(CUR)**13)+(LS_0**6)/(LENGTHS(CUR)**7))
