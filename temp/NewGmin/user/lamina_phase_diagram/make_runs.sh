...
#----------------------------------------------------------------------------------------------------------------
  ...
  L=5           # original length of filaments
 ...
 BE_ARRAY=(0.1) 
  ...
  USE_EXAMPLES=false # This is 'true' if we want the outputs of previous runs to be the inputs in our new runs
     ...
     for K3 in "${K3_ARRAY[@]}"
		 do
			...
			do
				...
				write_data
				echo "done"
				cp data.in /Users/katemcgarva/Desktop/NewGmin/user/lamina_phase_diagram/template/
				cd $runname
				cp ../../gmin .
				write_data
				cp ../../data.f90 .
