!lamina_new_clamped
subroutine set_defaults()
  use potential
  !#################!
  ! User Parameters !
  !#################!

  CM1 = 1.0D3 !Energy scale factor
 
  OPEN(1,FILE='/Users/katemcgarva/Desktop/NewGmin/user/lamina_phase_diagram/template/data.in')
  READ(1,*) N_ROD
  READ(1,*) N_SEG
  READ(1,*) L
  READ(1,*) H
  READ(1,*) K3
  READ(1,*) LOWER_RAT
  READ(1,*) F_I
  READ(1,*) F_S
  READ(1,*) BE
  CLOSE(1)
  !DOUBLE PRECISION, ALLOCATABLE :: BENDING_VAR(:,:) !do we need to do this


  R3=H/(N_ROD-1.0) !LC spring equilibrium length
  
  K1=K3 !Vertical spring stiffness
  R1=H/(N_ROD-1.0) !Vertical spring equilibrium length

  K2=K3  !Diagonal spring stiffness
  R2=SQRT( (L/N_SEG)**2 + (H/(N_ROD-1.0))**2 )  !Diagonal spring equilibrium length

   ALLOCATE(B_ARRAY(N_ROD))
  B_ARRAY(1)=0.01;
  B_ARRAY(N_ROD)=0.01
  
  DO i=2,N_ROD-1
 	  IF (MOD(i,2)==0) THEN
      B_ARRAY(i)=0.01 !Interior rod bending stiffness type1
    ELSE
      B_ARRAY(i)=0.01 !*LOWER_RAT !Interior rod bending stiffness type2
    END IF
  END DO
  
  ! ALLOCATE(B_ARRAY(N_ROD,N_SEG)) !Changed this

  ! B_ARRAY(1,:)=0.1;
  ! B_ARRAY(N_ROD,:)=0.1

  ! DO j=1,N_SEG !Changed this
  !   DO i=2,N_ROD-1
 	!     !IF ((MOD(i,2)==0) .AND. (MOD(j,2)==0)) THEN
  !     !IF (MOD(i,2)==0) THEN
  !       B_ARRAY(i,j)=0.01
  !     !ELSE
  !      !B_ARRAY(i,j)=0.01*LOWER_RAT !Interior rod bending stiffness type2
  !    !END IF
  !   END DO
  ! END DO

  !  B_ARRAY(1)=0.0;
  ! B_ARRAY(N_ROD)=0.0
  
  ! DO i=2,N_ROD-1
 	!   IF (MOD(i,2)==0) THEN
  !     B_ARRAY(i)=0.01 !Interior rod bending stiffness type1
  !   ELSE
  !     B_ARRAY(i)=0.01*LOWER_RAT !Interior rod bending stiffness type2
  !   END IF
  ! END DO
 

  KC=100000.0 !End constraint magnitude
  KY=10.0 !Overlap penalisation magnitude
  WY=0.001 !Overlap penalisation onset width
  KL=10 !10 !Segment length penalisation constant (axial stretching stiffness)

  KAREA=1.0D6 !Repulsion magnitude
  CAREA=100.0 !Repulsion exponential factor


  N = 2*(N_ROD)*(N_SEG-1) !multiplied by 2 for the array including ls as a variable (why n_seg-1)
  !N = SIZE(X)
  !###################!
  ! L-BFGS Parameters !
  !###################!

  M = 10
  max_iterations  = 1000000 !Max number of minimisation steps
  convergence_rms = 1d-5 !RMS gradient at convergence
  max_step_size   = 0.25d0
  H0init          = 1d-1
  relative_energy_check = .true.
  dE_max          = 1d-1
  debug = .true.

  !##########################!
  ! Basin Hopping Parameters !
  !##########################!

  max_runs          = 1
  max_similarity    = 1d-4
  temperature       = 1000d4
  binary_io         = .false.
  minima_save_limit = 0
end subroutine set_defaults
